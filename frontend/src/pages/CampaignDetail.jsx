...
export default function CampaignDetail() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 md:px-12 py-12" data-testid="campaign-detail">
      <Link to="/campaigns" className="text-sm text-[#525252] hover:text-[#E84A27] flex items-center gap-1 mb-6"><ArrowLeft size={14}/> Back to campaigns</Link>

      <div className="bg-white border border-[#E5E5E5] rounded-2xl p-8">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs px-2 py-1 bg-[#1A4331]/10 text-[#1A4331] rounded-full font-semibold">LIVE CAMPAIGN</span>
          <span className="text-xs text-[#A3A3A3]">by {c.brand_name}</span>
        </div>
        <h1 className="font-display text-5xl tracking-tight">{c.title}</h1>
        <p className="text-lg text-[#525252] mt-4 whitespace-pre-line">{c.description}</p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 border-t border-[#E5E5E5] pt-6">
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">{(c.deliverables||[]).map(d => <span key={d} className="px-3 py-1 bg-[#F7F5F2] text-sm rounded-full">{d}</span>)}</div>
        </div>
      </div>

      {/* Apply / Applicants */}
      {!isOwner && user?.role === "creator" && (
        <div className="bg-white border border-[#E5E5E5] rounded-2xl p-8 mt-6" data-testid="apply-section">
      ...
      {isOwner && (
        <div className="bg-white border border-[#E5E5E5] rounded-2xl p-8 mt-6">
          ...
          <div className="space-y-3" data-testid="applicants-list">
            {(c.applicants||[]).length === 0 && <div className="text-sm text-[#A3A3A3]">No applications yet.</div>}
            {(c.applicants||[]).map((a) => (
              <div key={a.application_id} className="p-4 border border-[#E5E5E5] rounded-xl">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="font-display text-lg">{a.creator_name}</div>
                    <p className="text-sm text-[#525252] mt-1">{a.pitch}</p>
                  </div>
