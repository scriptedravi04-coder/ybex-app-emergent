      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/collabs" className="text-sm text-[#E84A27] font-semibold">View all</Link>
          </div>
          <div className="space-y-3" data-testid="dashboard-collabs">
            {(isCreator ? collabs.received : collabs.sent).slice(0, 4).map(c => (
              <div key={c.collab_id} className="flex items-center justify-between p-3 border border-[#E5E5E5] rounded-xl">
                <div>
                  <div className="font-medium">{c.from_name}</div>
                  <div className="text-xs text-[#A3A3A3]">{c.deliverable} · ₹{(c.proposed_amount||0).toLocaleString("en-IN")}</div>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full ${c.status === "pending" ? "bg-[#F59E0B]/10 text-[#F59E0B]" : c.status === "accepted" ? "bg-[#1A4331]/10 text-[#1A4331]" : "bg-[#DC2626]/10 text-[#DC2626]"}`}>{c.status}</span>
              </div>
            ))}
            {(isCreator ? collabs.received : collabs.sent).length === 0 && <div className="text-sm text-[#A3A3A3] py-4 text-center">No items yet</div>}
          </div>
        </div>
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/campaigns" className="text-sm text-[#E84A27] font-semibold">View all</Link>
          </div>
          <div className="space-y-3">
            {campaigns.slice(0,4).map((c)=>(
              <Link to={`/campaigns/${c.campaign_id}`} key={c.campaign_id} className="block p-3 border border-[#E5E5E5] rounded-xl hover:border-[#E84A27]">
                <div className="font-medium">{c.title}</div>
                <div className="text-xs text-[#525252] mt-0.5">₹{(c.budget_min||0).toLocaleString("en-IN")} - ₹{(c.budget_max||0).toLocaleString("en-IN")} · {c.brand_name}</div>
              </Link>
            ))}
            {campaigns.length === 0 && <div className="text-sm text-[#A3A3A3] py-4 text-center">No campaigns yet</div>}
          </div>
        </div>
      </div>
    </div>
...
const StatBlock = ({ icon, label, value, accent }) => (
  <div className={`p-6 rounded-2xl border ${accent ? "bg-[#0A0A0A] text-white border-[#0A0A0A]" : "bg-white border-[#E5E5E5]"} hover:-translate-y-0.5 transition-transform`}>
    <div className="flex items-center justify-between"><div className={accent ? "text-[#E84A27]" : "text-[#E84A27]"}>{icon}</div></div>
