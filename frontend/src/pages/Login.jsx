...
export default function Login() {
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-16" data-testid="login-page">
      ...
      <p className="text-[#525252] mt-2">Log in to manage your collabs and campaigns.</p>

      <button onClick={handleGoogle} data-testid="google-login-btn" className="mt-8 w-full bg-white border border-[#0A0A0A] py-3 font-semibold hover:bg-[#0A0A0A] hover:text-white transition-colors flex items-center justify-center gap-2">
      ...
      <div className="flex items-center gap-4 my-6">
        ...
        <span className="text-xs text-[#A3A3A3] tracking-widest">OR</span>
        <div className="flex-1 h-px bg-[#E5E5E5]"></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input data-testid="login-email" type="email" required value={email} onChange={(e)=>setEmail(e.target.value)} className="input-field" placeholder="you@example.com"/>
        </div>
        <div>
          <label className="label-mini block mb-1.5">Password</label>
          <input data-testid="login-password" type="password" required value={password} onChange={(e)=>setPassword(e.target.value)} className="input-field" placeholder="Your password"/>
        </div>
        <button type="submit" data-testid="login-submit" disabled={loading} className="btn-primary w-full">{loading ? "Logging in..." : "Log In"}</button>
      </form>

      <p className="mt-6 text-sm text-[#525252]">New here? <Link to="/signup" className="text-[#E84A27] font-semibold">Create an account</Link></p>
    </div>
  );
}
